...
export class CoreService implements OnModuleInit, OnModuleDestroy {
  ...
  async reports(): Promise<ReportSummary> {
    ...
    return {
      ...
      teacherPayroll: aggregateTeacherPayroll(state.lessons),
      reminders: [
  ...
  async createOrder(input: Partial<Order>, meta: MutationMeta = {}): Promise<AppSnapshot> {
    ...
    const nextState = applyLedgerReadModels({ ...state, orders: [order, ...state.orders], paymentLedgerEntries });
  ...
  async recordPayment(orderId: string, meta: MutationMeta = {}): Promise<AppSnapshot> {
    return this.withMutation("recordPayment", { orderId }, meta, async (context) => {
	    ...
	    const paymentLedgerEntries: PaymentLedgerEntry[] = remainingAmount > 0 ? [
	      {
	        id: makeId("pay-ledger"),
	        orderId: order.id,
	        studentId: order.studentId,
      ...
      const nextState = applyLedgerReadModels({ ...state, orders, paymentLedgerEntries });
  ...
  async createKnowledgeDoc(input: { title: string; scope: string; content?: string }, context: RequestContext) {
    const previous = await this.snapshot();
    const chunks = chunkKnowledgeContent(input.content ?? input.title).map((content, index): KnowledgeChunk => ({
      id: makeId("chunk"),
      docId: "",
      chunkIndex: index,
      title: input.title,
      ...
      metadata: { createdBy: context.userId, source: "manual" },
    }));
    const doc = {
      id: makeId("doc"),
      title: input.title,
    ...
    return this.saveWithAudit(next, "创建知识文档", `已添加知识文档：${doc.title}`, "已完成", context.actorName, previous);
  }
  ...
  async searchKnowledge(query: string, limit = 5, context: RequestContext) {
    const state = await this.snapshot();
    const normalizedLimit = clamp(Number(limit || 5), 1, 20);
    const docById = new Map(state.ragDocs.map((doc) => [doc.id, doc]));
    const scoredChunks = state.knowledgeChunks
  ...
  async teacherAvailability(teacher: string, dateRange?: { from: string; to: string }, context: RequestContext = defaultRequestContext()) {
    const state = await this.snapshot();
...
function aggregateTeacherPayroll(lessons: Lesson[]): Array<{ teacher: string; lessons: number; pay: number }> {
